...
public class JWT_Encrypted_Validator_Callout implements Execution {
    ...
    private String getVarname(String stepName, String label) {
        String varName = "jwt_" + stepName + "_" + label;
        return varName;
    }


    private String getStepname() throws Exception {
    ...
    public ExecutionResult execute (MessageContext msgCtxt,
        ...
        // diagnostic purposes
        varName = getVarname(stepName,"jwt");
        msgCtxt.setVariable(varName, encryptedJwt);

        RSAPrivateKey privateKey = (RSAPrivateKey) getPrivateKey(msgCtxt);
        BASE64Encoder b64 = new BASE64Encoder();
        //System.out.println("Private Key :: " + b64.encode(privateKey.getEncoded()));
        varName = getVarname(stepName,"PrivateKey");
        ...
        varName = getVarname(stepName,"receivedClaimsJSON");
        msgCtxt.setVariable(varName, receivedClaimsJSON);
        }
        catch (Exception e) {
            e.printStackTrace();
            varName = getVarname(stepName, "error");
            msgCtxt.setVariable(varName, "Exception (A): " + e.toString());
            varName = getVarname(stepName, "stacktrace");
