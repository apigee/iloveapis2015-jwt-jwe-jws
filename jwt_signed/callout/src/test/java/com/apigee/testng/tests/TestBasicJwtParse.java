...
public class TestBasicJwtParse {
    ...
    public void test2_Rs256JwtMissingPemfileProperty_NoVerify() {
        String[][] cases = new String[][] {
            new String[] { null, "must specify pemfile or public-key or certificate when algorithm is RS*"},
            new String[] { "true", "must specify pemfile or public-key or certificate when algorithm is RS*"},
        ...
        for(String[] s : cases) {
            ...
            ExecutionResult expectedResult = ("false".equals(wantVerifyString)) ?
                ExecutionResult.SUCCESS : ExecutionResult.ABORT ;
            Map properties = new HashMap();
            properties.put("algorithm", "RS256");
            //properties.put("debug", "true"); // causes exception to be logged to stdout
            ...
            // check result and output
            String thisCase = String.format("Case(%s)", wantVerifyString!=null ? wantVerifyString : "null");
    ...
    public void test5_SimpleClaims2() {
        ...
        for(String continueOnErrorString : cases) {
            ...
            // check result and output
            String thisCase = String.format("Case(%s)", continueOnErrorString!=null ? continueOnErrorString : "null");
            Assert.assertEquals(result, expectedResult, thisCase);
            Assert.assertEquals(isValid, "false", thisCase);
            Assert.assertEquals(reason, expectedReason, thisCase);
        }
    ...
    public void test7_BadHmacKey_NoVerify() {
        ...
        for (String[] s : cases) {
            ExecutionResult expectedResult = ("false".equals(s[0])) ?
                ExecutionResult.SUCCESS : ExecutionResult.ABORT ; // SUCCESS ;
            Map properties = new HashMap();
            properties.put("algorithm", "HS256");
            properties.put("debug", "true");
            properties.put("jwt", jwtMap.get("sample1"));
